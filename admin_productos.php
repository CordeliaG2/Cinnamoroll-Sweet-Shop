...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nombre'])) {
    ...
    // Inserta el producto en la base de datos (sin imagen aún)
    ...
    if ($result) {
        ...
        // Crear un directorio para almacenar las imágenes del producto
        ...
        if (isset($_FILES['imagen_principal']) && $_FILES['imagen_principal']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!move_uploaded_file($_FILES['imagen_principal']['tmp_name'], $img_principal)) {
                echo "Error al subir la imagen principal.";
            }
        } else {
            echo "No se subió la imagen principal o ocurrió un error.";
        }

        // Procesar imágenes secundarias (si existen)
        if (isset($_FILES['imagenes_secundarias'])) {
            foreach ($_FILES['imagenes_secundarias']['tmp_name'] as $key => $tmp_name) {
                if ($_FILES['imagenes_secundarias']['error'][$key] === UPLOAD_ERR_OK) {
                    $ext      = pathinfo($_FILES['imagenes_secundarias']['name'][$key], PATHINFO_EXTENSION);
                    $filename = $ruta_producto . 'secundaria_' . uniqid() . '.' . $ext;
                    if (!move_uploaded_file($tmp_name, $filename)) {
                        echo "Error al subir la imagen secundaria $key.";
                    }
                }
            }
        }

        // Actualizar la base de datos con la ruta de la imagen principal
        $sql = $con->prepare("UPDATE productos SET imagen = ? WHERE id = ?");
        $sql->execute([$img_principal, $id_producto]);

        echo "Producto registrado correctamente. ID: " . $id_producto;
    } else {
...
<body>
  <header class="navbar navbar-expand-lg navbar-dark bg-sky-blue shadow-sm">
    <div class="container">
      <!-- Aquí puedes colocar el contenido del header -->
    </div>
  </header>
    ...
    <table class="table table-striped">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><?php echo isset($producto['stock']) ? htmlspecialchars($producto['stock']) : 'Sin definir'; ?></td>
          <td>
            <?php 
